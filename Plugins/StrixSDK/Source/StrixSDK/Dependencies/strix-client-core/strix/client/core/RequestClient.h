...
protected:
	...
	{
		std::weak_ptr<RequestClient> weakSelf = shared_from_this();

		ResponseHandler newHandler = [defaultHandler, handler, weakSelf](request::RequestResult<message::ResponseMessagePtr> requestResult) -> void {
			...
			{
				self->m_requestIds.erase(requestResult.m_result->GetRequestId());
			}
		...
		request::RequestFailureHandler newFailureHandler = (!defaultFailureHandler) ? failureHandler
			: [defaultFailureHandler, failureHandler, weakSelf](request::RequestResult<message::FailureMessagePtr> requestResult) -> void {
