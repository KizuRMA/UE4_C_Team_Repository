...
#include "Util/StrixHeapDataHolder.h"
...
struct FStrixPropertyMap
{
	...
	static std::map<std::string, strix::net::object::ObjectPtr> CreateNetworkPropertyMap(const FStrixPropertyMap& strixPropertyMap, const std::set<std::string> knownNetworkFieldNames)
	{
		...
		for (auto &entry : strixPropertyMap.GetPropertyMap())
		{
	...
	const std::map<std::string, strix::net::object::ObjectPtr> &GetPropertyMap() const { return m_propertyMap.Get(); }
	std::map<std::string, strix::net::object::ObjectPtr> &GetPropertyMap() { return m_propertyMap.Get(); }

	StrixHeapDataHolder<std::map<std::string, strix::net::object::ObjectPtr>> m_propertyMap;
};
