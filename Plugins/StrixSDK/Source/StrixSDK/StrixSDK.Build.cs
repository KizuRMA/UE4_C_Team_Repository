...
public class StrixSDK : ModuleRules
{
	public StrixSDK(ReadOnlyTargetRules Target) : base(Target)
	{
        ...
        {
			AddDependencyLibrariesCrossCompile(new List<String>() { "linux", "clang" });
        }
		else if (targetPlatform == UnrealTargetPlatform.Android)
        {

#if UE_4_25_OR_LATER
		AddDependencyLibraries(new List<String>() { "android", "ndk-21", "arm64-v8a" });
		AddDependencyLibraries(new List<String>() { "android", "ndk-21", "armeabi-v7a" });
		AddDependencyLibraries(new List<String>() { "android", "ndk-21", "x86" });
		AddDependencyLibraries(new List<String>() { "android", "ndk-21", "x86_64" });
#else
		AddDependencyLibraries(new List<String>() { "android", "ndk-161","arm64-v8a" });
		AddDependencyLibraries(new List<String>() { "android", "ndk-161", "armeabi-v7a" });
		AddDependencyLibraries(new List<String>() { "android", "ndk-161", "x86" });
		AddDependencyLibraries(new List<String>() { "android", "ndk-161", "x86_64" });
#endif
	}
	...
	private void AddDependencyLibrariesCrossCompile(List<String> pathParts)
	{
		String path = Path.Combine(ModuleDirectory, "Lib");
		foreach (String pathPart in pathParts)
		{
