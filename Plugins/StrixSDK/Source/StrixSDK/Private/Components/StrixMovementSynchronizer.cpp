...
const std::string UStrixMovementSynchronizer::NearDistancePropertyName = "StrixMovementSynchronizer.NearDistance";
const std::string UStrixMovementSynchronizer::MaxSpeedPropertyName = "StrixMovementSynchronizer.MaxSpeed";
const std::string UStrixMovementSynchronizer::AccelerationPropertyName = "StrixMovementSynchronizer.Acceleration";
const std::string UStrixMovementSynchronizer::DampingPropertyName = "StrixMovementSynchronizer.Damping";
const std::string UStrixMovementSynchronizer::ReachableDistancePropertyName = "StrixMovementSynchronizer.ReachableDistance";
...
void UStrixMovementSynchronizer::SetNearDistance(float distance)
{
	if (m_pMovementController)
	{
		NearDistance = distance;
		m_pMovementController->SetNearDistance(distance);
	}
}

void UStrixMovementSynchronizer::SetMaxSpeed(float maxSpeed)
{
	if (m_pMovementController)
	{
		MaxSpeed = maxSpeed;
		m_pMovementController->SetMaxSpeed(maxSpeed);
	}
}

void UStrixMovementSynchronizer::SetAcceleration(float acceleration)
{
	if (m_pMovementController)
	{
		Acceleration = acceleration;
		m_pMovementController->SetAcceleration(acceleration);
	}
}

void UStrixMovementSynchronizer::SetDamping(float damping)
{
	if (m_pMovementController)
	{
		Damping = damping;
		m_pMovementController->SetDamping(damping);
	}
}

void UStrixMovementSynchronizer::SetReachableDistance(float distance)
{
	if (m_pMovementController)
	{
		ReachableDistance = distance;
		m_pMovementController->SetReachableDistance(distance);
	}
}
...
void UStrixMovementSynchronizer::CreatePropertiesSyncGroup()
{
	if (m_replicator)
	{
		...
		m_syncGroup->AddProperty(std::make_shared<UESyncProperty<float>>(NearDistancePropertyName, m_pMovementController->GetNearDistanceRef()));
		m_syncGroup->AddProperty(std::make_shared<UESyncProperty<float>>(MaxSpeedPropertyName, m_pMovementController->GetMaxSpeedRef()));
		m_syncGroup->AddProperty(std::make_shared<UESyncProperty<float>>(AccelerationPropertyName, m_pMovementController->GetAccelerationRef()));
		m_syncGroup->AddProperty(std::make_shared<UESyncProperty<float>>(DampingPropertyName, m_pMovementController->GetDampingRef()));
		m_syncGroup->AddProperty(std::make_shared<UESyncProperty<float>>(ReachableDistancePropertyName, m_pMovementController->GetReachableDistanceRef()));
