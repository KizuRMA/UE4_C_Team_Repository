	...
	else
	{
		...
		if (!IsStaticWorldObject())
		{
			...
			if (castedArchetype)
			{
				...
				// if saved TypeId != this objects TypeId , it should rewrite.
				else if (castedArchetype->TypeId != TypeId)
				{
					castedArchetype->TypeId = TypeId;
				}
			}
...
FStrixRoomMember UStrixReplicatorComponent::GetRoomMember()
{
  if(UStrixNetworkFacade* facade = UStrixNetworkFacade::GetInstance(this)) {
    if (m_networkObject) {
      strix::client::core::UIDPtr ownerUid = m_networkObject->GetOwnerUid();
      return facade->GetRoomMemberByUID(ChannelId, ownerUid);
    }
  }
