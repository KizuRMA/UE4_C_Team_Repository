...
SyncPropertyPtr UESyncPropertyUtils::GetUESyncProperty(UTextProperty* textProperty, void* object, const std::string &name)
{
	...
	return std::make_shared<UEFunctionalSyncProperty<std::string>>(
		[=]()
		{
			...
			[=](std::string value) {
			*target = FText::FromString(FString(UTF8_TO_TCHAR(value.c_str())));
		},
		name);
}
...
SyncPropertyPtr UESyncPropertyUtils::GetUESyncProperty(UNameProperty* nameProperty, void* object, const std::string &name)
{
	...
	return std::make_shared<UEFunctionalSyncProperty<std::string>>(
		[=]()
			...
			[=](std::string value)
			{
				*target = FName(UTF8_TO_TCHAR(value.c_str()));
			},
		name);
}
...
SyncPropertyPtr UESyncPropertyUtils::GetUESyncProperty(UArrayProperty* arrayProperty, void* object, const std::string &name)
{
	...
	if (innerClass->IsChildOf(UStrProperty::StaticClass()))
	{
		auto target = arrayProperty->ContainerPtrToValuePtr<TArray<FString>>(object);
		return std::make_shared<UEArraySyncProperty<FString>>(name, *target);
	}
	if (innerClass->IsChildOf(UTextProperty::StaticClass()))
	{
		auto target = arrayProperty->ContainerPtrToValuePtr<TArray<FText>>(object);
		return std::make_shared<UEArraySyncProperty<FText>>(name, *target);
	}
	if (innerClass->IsChildOf(UNameProperty::StaticClass()))
	{
		auto target = arrayProperty->ContainerPtrToValuePtr<TArray<FName>>(object);
		return std::make_shared<UEArraySyncProperty<FName>>(name, *target);
	}
	return nullptr;
}
