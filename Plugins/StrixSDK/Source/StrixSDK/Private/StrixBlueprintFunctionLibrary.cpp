...
{
	auto mapProperty = strixStructure.GetPropertyMap().find(TCHAR_TO_UTF8(*propertyName));
	if (mapProperty == strixStructure.GetPropertyMap().end())
		return false;
...
static FStrixPropertyMap SetProperty(FStrixPropertyMap &strixStructure, const FString &propertyName, const T &value)
{
	...
	auto result = strixStructure.GetPropertyMap().insert(std::make_pair(convertedPropertyName, obj));
...
int UStrixBlueprintFunctionLibrary::GetSyncTimePeriod(const UObject* WorldContextObject, int channelId)
{
	UStrixNetworkFacade* strixNetwork = UStrixNetworkFacade::GetInstance(WorldContextObject);
	if(strixNetwork)
	{
		return strixNetwork->GetSyncTimePeriod(channelId);
	}
	return -1;
}

void UStrixBlueprintFunctionLibrary::SetSyncTimePeriod(const UObject* WorldContextObject, int channelId, int period)
{
	UStrixNetworkFacade* strixNetwork = UStrixNetworkFacade::GetInstance(WorldContextObject);
	if (strixNetwork)
	{
		return strixNetwork->SetSyncTimePeriod(channelId, period);
	}
}

int UStrixBlueprintFunctionLibrary::GetSyncRTT(const UObject* WorldContextObject, int channelId)
{
	UStrixNetworkFacade* strixNetwork = UStrixNetworkFacade::GetInstance(WorldContextObject);
	if (strixNetwork)
	{
		return strixNetwork->GetSyncRTT(channelId);
	}
	return -1;
}
...
FDateTime UStrixBlueprintFunctionLibrary::GetSynchronizedTime(const UObject* WorldContextObject, int32 channelId)
{
	UStrixNetworkFacade* strixNetwork = UStrixNetworkFacade::GetInstance(WorldContextObject);
	return  strixNetwork->GetSynchronizedTime(channelId);
}
...
FStrixRelayArg UStrixBlueprintFunctionLibrary::Conv_IntKeyBoolMapToStrixRelayArg(TMap<int32, bool>val)
{
	FStrixRelayArg arg(strix::net::object::UEObjectAdapter(val).Get());
...
FStrixRelayArg UStrixBlueprintFunctionLibrary::Conv_StringKeyBoolMapToStrixRelayArg(TMap<FString, bool>val)
{
	FStrixRelayArg arg(strix::net::object::UEObjectAdapter(val).Get());
