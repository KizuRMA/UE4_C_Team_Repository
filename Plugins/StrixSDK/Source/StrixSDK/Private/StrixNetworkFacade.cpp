...
#include <strix/net/serialization/Serialization.h>
#include "strix/client/core/auth/UIDCredentials.h"
#include <strix/client/core/time/SyncTimeClient.h>
...
FDateTime UStrixNetworkFacade::GetSynchronizedTime(int channedlId)
{
	const auto roomContext = GetRoomContextByChannelId(channedlId);
	if (roomContext)
	{
		auto ptr = roomContext->GetSyncTimeClient();
		if (ptr)
		{
			auto syncTime = ptr->GetSynchronizedTime();
			auto sec = std::chrono::duration_cast<std::chrono::duration<long long, std::ratio<1, 10000000>>>(syncTime.time_since_epoch()).count();
			return FDateTime(1970, 1, 1) + FTimespan(sec);
		}
	}
	return FDateTime(1970, 1, 1) + FTimespan(std::chrono::system_clock::now().time_since_epoch().count());
}
	...
	const FString &password, bool pauseNetworkObjects, const FStrixRoomContextOpenDelegate& successCallback, const FStrixErrorCodeFailureDelegate& failureCallback)
{
	...
	long long roomId = roomInfo.NodeRoom.RoomId;
...
strix::client::core::util::DelegateList<int>::Handle UStrixNetworkFacade::AddRoomServerDisconnectHandler(RoomServerDisconnectHandler handler)
{
	return m_roomServerDisconnectDelegate.Add(std::move(handler));
}

void UStrixNetworkFacade::RemoveRoomServerDisconnectHandler(strix::client::core::util::DelegateList<>::Handle handle)
{
	m_roomServerDisconnectDelegate.Remove(handle);
}
...
void UStrixNetworkFacade::InitBase(UGameInstance* gameInstance)
{
	...
	FConfigCacheIni::LoadGlobalIniFile(TypeIdConfigFilename, TEXT("Game"), 0, true, false, true, true, *FPaths::ProjectConfigDir());
#else
	...
	///For fix Windows encording problems.
	strix::net::serialization::SerializationConfig serializationConfig;
	serializationConfig.SetEncodingConverter(nullptr);
	strix::net::serialization::Serialization::GetSerializationManager().SetConfig(std::move(serializationConfig));
...
{
	if (m_strixClient) {
		// Increasing the shared_ptr reference counter temporarily
		// since StrixNetworkFacade can be destroyed from within the Process method
		auto tempStrixClient = m_strixClient;
		tempStrixClient->Process();
	}
}
...
void UStrixNetworkFacade::StopBase()
{
	...
	if (m_strixClient)
	{
		...
		m_roomConnectionsByChannelId.clear();
		m_roomContextsByChannelId.clear();
...
bool UStrixNetworkFacade::TryAddRoomConnection(int32 channelId,	strix::client::ingame::network::RoomServerConnectionPtr roomConnection)
{
	...
	m_roomConnectionsByChannelId.insert(std::make_pair(channelId, roomConnection));
		...
		[=](strix::client::ingame::network::RoomServerConnectionClosedEventArgs args) {
			m_roomServerDisconnectDelegate.Call(channelId);
...
FStrixRoomMember UStrixNetworkFacade::GetRoomMemberByUID(int channelId, strix::client::core::UIDPtr uid) {
	strix::client::room::model::RoomMemberMap members;
	auto context = m_roomContextsByChannelId.find(channelId);
	if (context != m_roomContextsByChannelId.end())
	{
...
long long UStrixNetworkFacade::GetSyncTimePeriod(int channelId)
{
	auto context = GetRoomContextByChannelId(channelId);
	if(context)
	{
		return context->GetSyncTimeClient()->GetSyncPeriod();
	}
	return -1;
}

void UStrixNetworkFacade::SetSyncTimePeriod(int channelId, long long period)
{
	auto context = GetRoomContextByChannelId(channelId);
	if (context)
	{
		context->GetSyncTimeClient()->SetSyncPeriod(period);
	}
}

long long UStrixNetworkFacade::GetSyncRTT(int channelId)
{
	auto context = GetRoomContextByChannelId(channelId);
	if (context)
	{
		return context->GetSyncTimeClient()->GetRTT();
	}
	return -1;
}
